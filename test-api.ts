...
function loadConfig() {
  ...
  if (!baseUrl || !email || !apiToken) {
    ...
    console.error("  CONFLUENCE_BASE_URL:", baseUrl ? "✅ 設定済み" : "❌ 未設定");
    console.error("  CONFLUENCE_EMAIL:", email ? "✅ 設定済み" : "❌ 未設定");
    console.error("  CONFLUENCE_API_TOKEN:", apiToken ? "✅ 設定済み" : "❌ 未設定");
    console.error("");
...
async function testConnection(client: ConfluenceClient) {
  ...
  try {
    // 最も軽いAPIエンドポイントをテスト
    const response = await fetch(`${client['config'].baseUrl}/wiki/api/v2/spaces?limit=1`, {
    ...
    if (response.ok) {
      ...
      console.error("❌ API接続エラー:", response.status, errorText.substring(0, 200));
      return false;
    }
  } catch (error) {
...
async function main() {
  ...
  console.log(`  API Token: ${'*'.repeat(Math.min(config.apiToken.length, 20))}...\n`);
